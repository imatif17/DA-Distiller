...
def office_m_concat_split_loader(paths, batch_size=16, num_workers=1, pin_memory=False, drop_last=False, n_splits=3):
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    splits = np.ones(n_splits) * (len(dataset) // n_splits)
    splits = splits.astype(int).tolist()
    splits[-1] = splits[-1] + len(dataset) % n_splits
    dataset_splits = torch.utils.data.random_split(dataset, splits)

    dataloaders = []
    for dt in dataset_splits:
        dt.num_classes = len(datasetsF[0].classes)
...
def office_m_concat_split_test_loader(target_paths, batch_size=16, num_workers=1, pin_memory=False, n_splits=3):
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    datasetsF = []
...
def get_source_split_concat_target_loader(dataset_name, source_path, target_paths, batch_size=16, num_workers=1, pin_memory=False, drop_last=False, n_splits=3, **kwargs):


    source_dataloader = None
    targets_testloader = [0] * len(target_paths)
    if dataset_name == "Office31":
        if source_path != "":
            source_dataloader = office_loader(source_path, batch_size, num_workers, pin_memory, drop_last)
        target_dataloaders = office_m_concat_split_loader(target_paths, batch_size, num_workers, pin_memory, drop_last, n_splits)
        for i, p in enumerate(target_paths):
            targets_testloader[i] = office_test_loader(p, batch_size, num_workers, pin_memory)
        target_testloaders = office_m_concat_split_test_loader(target_paths, batch_size, num_workers, pin_memory, n_splits)
    elif dataset_name == "ImageClef":
        if source_path != "":
            source_dataloader = imageclef_train_loader(source_path, batch_size, num_workers, pin_memory, drop_last)
        ...
        target_dataloaders, target_testloaders = get_digit_five_train_loader_con_split(target_paths, batch_size, num_workers, pin_memory, drop_last, n_splits, kwargs['resize'])
    else:
        raise("Dataset not handled")


    return source_dataloader, target_dataloaders, target_testloaders
